...
document.addEventListener('keypress', function(e) {
    if (document.activeElement.id === 'newTodo' && e.key === 'Enter') addTodo();
});
//将待做事项列表保存到本地存储中
...
function loadData() {
    ...
    if (record !== null) return JSON.parse(record);
    else return [];
}
//页面渲染 主函数 获取页面需要更新的所有数据,当数据更新时将页面重新渲染
function render() {
    let todo = document.querySelector('.todoList'), done = document.querySelector('.doneList'), left = document.querySelector('.leftCount'), todoString = '', doneString = '', leftCount = 0;
    document.querySelector('#newTodo').focus();
    todoList = loadData();
    if (todoList.length === 0) document.querySelector('.mask').style.display = 'flex';
    else document.querySelector('.mask').style.display = 'none';
    if (todoList !== null) {
        todoList.map((e)=>{
            if (!e.done) {
...
function seekIndex(seekId) {
    todoList.forEach((e)=>{
        if (e.id === seekId) workIndex = todoList.indexOf(e);
    });
}
...
function clearDone() {
    todoList = todoList.filter((e)=>e.done === false
    );
    update(todoList);
}
...
function fnDate() {
    ...
    let detailTime = year + '-' + fnW(month + 1) + '-' + fnW(data) + ' ' + fnW(hours) + ':' + fnW(minute) + ':' + fnW(second);
...
//# sourceMappingURL=index.ea850909.js.map
